                        ...
                        if 0 <= row < config.ROWS and 0 <= col < config.COLS:
                            ...
                            if selected_square == (row, col):
                                ...
                                if len(player_clicks) == 2:
                                    start_row = 7 - (player_clicks[0] // 8)
                                    start_col = player_clicks[0] % 8
                                    moved_piece_str = gs.board[start_row][start_col]
                                    
                                    if moved_piece_str != "--":
                                        p_char = moved_piece_str[2] if moved_piece_str[2] != 'n' else 's'
                                        if moved_piece_str[2] == 'n': p_char = 'n'
                                        p_char = p_char.upper() if moved_piece_str[0] == 'w' else p_char.lower()
                                    else:
                                        p_char = "--"

                                    move = Move(player_clicks[0], player_clicks[1], piece_moved=p_char)
                                    found_move = next((m for m in valid_moves if move == m), None)

                                    if found_move:
                                        if found_move.is_promotion:
