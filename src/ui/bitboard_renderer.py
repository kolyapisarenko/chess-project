...
class BoardRenderer:
    def __init__(self, screen, font):
        ...
        light_img = pygame.image.load(config.CELL_PATH + "square_brown_light.png")
        dark_img = pygame.image.load(config.CELL_PATH + "square_brown_dark.png")
        analyse_light_img = pygame.image.load(config.CELL_PATH + "square_gray_light.png")
        analyse_dark_img = pygame.image.load(config.CELL_PATH + "square_gray_dark.png")
    ...
    def draw_arrows(self, arrows):
        ...
        for arrow in arrows:
            start_row, start_col = arrow[0][0], arrow[0][1]
            end_row, end_col = arrow[1][0], arrow[1][1]
    ...
    def _draw_selected_square(self, game_state, selected_square, valid_moves):
        if len(game_state.move_log) > 0:
            ...
            start_row, start_col = last_move.start_sq // 8, last_move.start_sq % 8
            end_row, end_col = last_move.target_sq // 8, last_move.target_sq % 8
            ...
            start_x = config.ORIGIN_X + (start_col * config.CELL_SIZE)
            start_y = config.ORIGIN_Y + (start_row * config.CELL_SIZE)
            self.screen.blit(history_surface, (start_x, start_y))

            end_x = config.ORIGIN_X + (end_col * config.CELL_SIZE)
            end_y = config.ORIGIN_Y + (end_row * config.CELL_SIZE)
            self.screen.blit(history_surface, (end_x, end_y))

        if selected_square:
            row, col = selected_square
            if game_state.board[row][col] != "--" and game_state.board[row][col] != "":
                x = config.ORIGIN_X + (col * config.CELL_SIZE)
                y = config.ORIGIN_Y + (row * config.CELL_SIZE)
        ...
        if selected_square:
            ...
            if piece != "--" and piece != "":
                ...
                if is_piece_white == game_state.white_to_move:
                    for move in valid_moves:
                        move_start_row, move_start_col = move.start_sq // 8, move.start_sq % 8
                        move_end_row, move_end_col = move.target_sq // 8, move.target_sq % 8

                        if move_start_row == row and move_start_col == col:
                            center_x = config.ORIGIN_X + (move_end_col * config.CELL_SIZE)
                            center_y = config.ORIGIN_Y + (move_end_row * config.CELL_SIZE)

                            target = game_state.board[move_end_row][move_end_col]
                            hint_color = (0, 0, 0, 80)
    ...
    def draw_promotion_menu(self, is_white):
        ...
        for i, piece_name in enumerate(pieces):
            piece_img_key = f"{color}_{piece_name}"
            img = self.piece_images[piece_img_key]
    ...
    def draw_move_log(self, game_state):
        ...
        for i in range(0, len(move_log), 2):
            ...
            if -config.LOG_LINE_HEIGHT < y_pos < config.SCREEN_HEIGHT:
                ...
                white_move = font.render(str(move_log[i]), True, (255, 255, 255))
                log_entries_surf.blit(white_move, (config.LOG_PADDING_LEFT + 40, y_pos))
                
                if i + 1 < len(move_log):
                    black_move = font.render(str(move_log[i+1]), True, (255, 255, 255))
    ...
    def draw_material_advantage(self, game_state):
        ...
        if black_adv or balance < 0:
            ...
            for b_piece in black_adv:
                file_key = self.char_to_file.get(b_piece.lower(), "b_pawn")
                self.screen.blit(self.material_piece_images[file_key], (x_pos, y_pos))
                x_pos += icon_size - 5
        ...
        if white_adv or balance > 0:
            ...
            for w_piece in white_adv:
                file_key = self.char_to_file.get(w_piece.upper(), "w_pawn")
                self.screen.blit(self.material_piece_images[file_key], (x_pos, y_pos))
                x_pos += icon_size - 5
    ...
    def draw_game_over(self, result_text, sub_text):
        ...
        restart_button_rect = pygame.Rect((config.SCREEN_WIDTH // 2) - button_w - (spacing // 2), btn_y, button_w, button_h)
        quit_button_rect = pygame.Rect((config.SCREEN_WIDTH // 2) + (spacing // 2), btn_y, button_w, button_h)
    ...
    def render(self, game_state, selected_square, valid_moves, highlighting_set, arrows):
        ...
        if game_state.checkmate or game_state.stealmate or game_state.on_time or game_state.check_insufficient_material():
            ...
            if game_state.checkmate:
                ...
                sub_text = f"{winner} wins by technical superiority"
            elif game_state.stealmate:
                res_txt = "STALEMATE"
                sub_text = "Draw: No legal moves"
                ...
                res_txt = "DRAW"
                sub_text = "Insufficient material to mate"
            return self.draw_game_over(res_txt, sub_text)