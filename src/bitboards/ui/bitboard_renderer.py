...
class BoardRenderer:
    ...
    def _draw_selected_square(self, game_state, selected_square, valid_moves):
        if len(game_state.move_log) > 0:
            ...
            start_row, start_col = 7 - (last_move.start_sq // 8), last_move.start_sq % 8
            end_row, end_col = 7 - (last_move.target_sq // 8), last_move.target_sq % 8
        ...
        if selected_square:
            ...
            piece = game_state.board[row][col]
            if piece != "--" and piece != "":
                is_piece_white = piece[0] == "w"
                if is_piece_white == game_state.white_to_move:
                    for move in valid_moves:
                        move_start_row, move_start_col = 7 - (move.start_sq // 8), move.start_sq % 8
                        move_end_row, move_end_col = 7 - (move.target_sq // 8), move.target_sq % 8
    ...
    def draw_promotion_menu(self, is_white):
        ...
        piece_rects = []
        for i, piece_name in enumerate(short_names):
            piece_img_key = f"{color}_{pieces[i]}"
